<script>
  ...
  const stateTaxPaidLastYear = 5000
  const incomeExpectationThisYear = 'same'
  const stateIncomeLastYear = 40000
  const expectedTotalIncomeThisYear = 120000
  const businessExpensesThisYear = 5000
  const retirementContributionsThisYear = 2000
  const studentLoanInterestThisYear = 500
  const healthInsuranceThisYear = 800
  const otherDeductionsThisYear = 700
  const exemptions = 1
  let stateWithholdings = 1800
  let q1PaymentMade = 400
...
<div class="page">
  ...
  <div class="section">
    ...
    <div>totalStateTaxLiability  = {formatCurrency(quarterlyPayment.totalStateTaxLiability )}</div>
